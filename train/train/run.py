...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    output_shape = dataset.get_num_labels()
    model = Sequential(
        [
    ...
    json_train_conf = json.dumps(train_conf)
    json_file_train_conf = open(os.path.join(artefacts_path, "params.json"), "w")
    json_file_train_conf.write(json_train_conf)
    json_file_train_conf.close()

    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.json
    dict_lab_ind = dataset.get_label_to_index_map()
    json_lab_ind = json.dumps(dict_lab_ind)
    json_file_lab_ind = open(os.path.join(artefacts_path, "labels_index.json"), "w")
    json_file_lab_ind.write(json_lab_ind)
    json_file_lab_ind.close()
