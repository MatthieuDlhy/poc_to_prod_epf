...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        # TODO: CODE HERE
        dict_index_2_label = {}
        list_label = self._get_label_list()
        for ind in range(self.get_num_labels()):
            dict_index_2_label[ind] = list_label[ind]
        return dict_index_2_label
    ...
    def get_label_to_index_map(self):
        ...
        """
        # TODO: CODE HERE
        dict_label_2_index = {value: key for key, value in self.get_index_to_label_map().items()}
        return dict_label_2_index
    ...
    def to_indexes(self, labels):
        ...
        """
        # TODO: CODE HERE
        list_of_indexes = [self.get_label_to_index_map()[label] for label in labels]
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        # TODO: CODE HERE
        assert (dataset.columns == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']).any()
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                # return df[df.tag_name.map(df["tag_name"].value_counts()) >= position]
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                tags_counts = df['tag_id'].value_counts()
                tags_to_remove = tags_counts[tags_counts < x]
                for tag in tags_to_remove:
                    df.drop(df[df['tag_id'] == tag], inplace=True)
        ...
        # TODO: CODE HERE
        result = dataset.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
        return result
