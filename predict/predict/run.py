...
class TextPredictionModel:
    ...
    def from_artefacts(cls, artefacts_path: str):
        ...
        # TODO: CODE HERE
        # load params
        params = json.load(os.path.join(artefacts_path, "params.json"))

        # TODO: CODE HERE
        # load labels_to_index
        labels_to_index = json.load(os.path.join(artefacts_path, "labels_index.json"))
    ...
    def predict(self, text_list, top_k=5):
        ...
        topk_sort_indexes = argsort(predict)[-top_k:]
        topk_tags_indexes = [predict[i] for i in topk_sort_indexes]
        predictions = [self.labels_index_inv[x] for x in topk_tags_indexes]
